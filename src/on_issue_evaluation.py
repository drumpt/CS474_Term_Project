...
def main(config):
    config["data_dir"] = "sample_dataset"
    ...
    vectorized_df = vectorizer.vectorize().head(30)

    config["clustering"]["method"] = "hierarchical"
    clusterizer = clustering.Clustering(vectorized_df, config)
    clustered_df = clusterizer.apply_clustering()
    clusterizer.evaluate()

    config["clustering"]["method"] = "DBSCAN"
    clusterizer = clustering.Clustering(vectorized_df, config)
    clustered_df = clusterizer.apply_clustering()
    clusterizer.evaluate()

    config["clustering"]["method"] = "OPTICS"
    clusterizer = clustering.Clustering(vectorized_df, config)
    clustered_df = clusterizer.apply_clustering()
    clusterizer.evaluate()
